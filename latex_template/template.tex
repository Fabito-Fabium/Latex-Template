...
\title{\Huge{Anotações}\\Exemplos Aleatórios}
\author{\huge{Seu nome}}
\date{}
...
\chapter{}
\section{Alguns Exemplos Aleatórios}
\indent
Nessa seção, veremos como podemos utilizar esse template, que tem como base o repositório de 
\href{https://github.com/SeniorMars/dotfiles}{SeniorMars}. 

\dfn{Limite de Sequências em $\bs{\bbR}$}{Seja $\{s_n\}$ uma sequência em $\bbR$. Dizemos que 
\begin{align*}
  \lim_{n\to\infty}s_n=s \text{,}
\end{align*}

com $s\in\bbR$, quando para cada $\eps>0$, existe um natural $N$ tal que 
dado um inteiro satisfazendo $n>N$, temos: $$s-\eps<s_n<s+\eps\quad\text{ i.e. }\quad |s-s_n|<\eps \text{.}$$}

\qs{}{Mostre que a bola aberta é um aberto.}

\sol Basta verificar que todo elemento da bola é um ponto interior dela.

\nt{Fiquei com preguiça de reescrever todo o resto do texto... MAS, é possível verificar que toda chamada 
de blocos está traduzido para o português (o que era o intuito do projeto).}
...
\mlemma{}{Suppose $\vec{v_1}, \dots, \vec{v_n} \in \RR[n]$ is subspace of $\bbR^n$.}
\mprop{}{$1 + 1 = 2$.}
